...
public class SumProductBP {
				...
				if (!alreadyAddedNodes.contains(node_j)) {
					if (hsj.size() == 1) {
							...
							alreadyAddedNodes.add(node_j);
							 System.out.println("Addnig ready node:" +
							 node_j);
	...
	private static void computeDelta_i_to_j(String node_i, String node_j) {
		...
		deltas.put(node_i + "_" + node_j, spMessage(node_i, node_j));

	}

	// returns potential of 1.
	private static double[] spMessage(String node_i, String node_j) {
		...
		delta_x_i_to_x_j[0] = product_of_deltas[0]
				* (phi_x_i_x_j[0] + phi_x_i_x_j[1]) * phi_x_i[0];
		delta_x_i_to_x_j[1] = product_of_deltas[1]
				* (phi_x_i_x_j[2] + phi_x_i_x_j[3]) * phi_x_i[1];
